...
export default function CreateOrderForm() {
  ...
  // Cargar clientes y tiendas al iniciar
  useEffect(() => {
    async function fetchData() {
      try {
        const clientesResponse = await fetch("/api/customers");
        const tiendasResponse = await fetch("/api/store");
  ...
  // Actualizar productos al cambiar tienda
  useEffect(() => {
    async function fetchProductos() {
      if (!formData.tienda) return;
      try {
        const response = await fetch(`/api/inventory/${formData.tienda}`);
        const inventoryData = await response.json();
        setProductos(inventoryData.productos.map((p) => p.producto));
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Encontrar el nombre de la tienda basado en el ID seleccionado
    const tiendaSeleccionada = tiendas.find(
      (tienda) => tienda._id === formData.tienda
    );

    // Crear un nuevo objeto para enviar al backend con el nombre de la tienda
    const requestData = {
      ...formData,
      tienda: tiendaSeleccionada?.nombre || formData.tienda, // Usar el nombre o fallback al ID
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        window.alert(errorData.message || "Error al crear la orden.");
      }
    } catch (error) {
      setMessage("Error al conectar con el servidor.");
    } finally {
