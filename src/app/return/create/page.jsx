...
export default function CreateReturnForm() {
  ...
  const [productos, setProductos] = useState([]); // Lista de productos del pedido
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState("");
  ...
  const fetchOrderProducts = async () => {
    ...
    try {
      const response = await fetch(`/api/order/${formData.pedido}`);
      if (response.ok) {
        ...
        const errorData = await response.json();
        setMessage(errorData.message || "No se pudieron cargar los productos.");
      }
    } catch (error) {
      setMessage("Error al conectar con el servidor.");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        window.alert("Devolución creada exitosamente.");
        setFormData({
          producto: "",
          pedido: "",
          cantidad: 1,
        ...
        const errorData = await response.json();
        window.alert(errorData.message || "Error al crear la devolución.");
      }
    } catch (error) {
      setMessage("Error al conectar con el servidor.");
    } finally {
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          ...
          <input
            ...
            onChange={handleInputChange}
            className="w-full border border-gray-300 rounded px-4 py-2"
            onBlur={fetchOrderProducts} // Buscar productos al salir del campo
            required
          />
        </div>
        ...
        {productos.length > 0 && (
          <div className="mb-4">
            ...
            >
              ...
              {productos.map((item) => (
                <option key={item.producto._id} value={item.producto._id}>
